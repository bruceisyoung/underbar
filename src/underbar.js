(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(memo,item,index,array){
             if(memo===false)
               return false;
             else
               if((iterator==undefined)?_.identity(item):iterator(item,index,array))
                 return true;
               else
                 return false;
           },true);
  ...
  _.memoize = function(func) {
    ...
    return function(){
      ...
      if(calculatedResult[key]==null){
        memoFunc = func.apply(this, arguments);
        calculatedResult[key] = memoFunc;
      }
      else
        memoFunc = calculatedResult[key];
      return memoFunc;
    };  
  };
  ...
  _.shuffle = function(array) {
    ...
    for(i=0; i<array.length; i++){
      ...
      result.push(temp[j]);
      temp=temp.slice(0,j).concat(temp.slice(j+1));
    }
